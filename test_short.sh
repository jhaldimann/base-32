...
nasm -f elf64 -g -F dwarf base32enc_short.asm -o b32es.o || { echo "Assembly code base32enc_short.asm failed to compile"; exit 1; }
ld -o b32es b32es.o || { echo "Object failed to link"; exit 1; }
...
do
  points=1
  timeout -s SIGKILL 1s echo -n $n | ./b32es > $n.out || { echo "Your 'b32' command failed to run: $?" ; points=0 ; }
...
points=1
timeout -s SIGKILL 1s cat base32enc_short.asm | ./b32es > text.out || { echo "Your 'b32' command failed to run: $?" ; points=0 ; }
cat base32enc_short.asm | base32  > text.want || { echo "System 'base32' failed to run"; exit 1; }
...
# Then we compare with the result produced by system 'base32'
points=1
timeout -s SIGKILL 1s cat b32es | ./b32es > binary.out || { echo "Your 'b32' command failed to run: $?" ; points=0 ; }
cat b32es | base32  > binary.want || { echo "System 'base32' failed to run"; exit 1; }
